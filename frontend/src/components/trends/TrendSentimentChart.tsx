...
import { TrendSentimentData, SentimentBucket } from '../../types/api';
...
export const TrendSentimentChart: React.FC<TrendSentimentChartProps> = ({
  ...
  // ECharts dual-axis option: stacked volume bars + net polarity score curve
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[11px] font-mono font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-900/40">
              RoBERTa Model Evaluated
            </span>
          </div>
          <p className="text-[12px] text-[#8591A5] dark:text-slate-400 font-medium mt-0.5">
            UTC chronological volume breakdown (Positive / Neutral / Negative) and mean polarity score over time
          </p>
        </div>
      ...
      {hasBuckets ? (
        ...
        <div className="h-[200px] rounded-[20px] bg-[#FAFBFD] dark:bg-[#12161C] border border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center p-6 text-center space-y-2">
          ...
          <p className="text-[12px] text-[#8591A5] dark:text-slate-400 max-w-sm">
            This Trend contains non-text or media-only items that do not contain RoBERTa sentiment inference records.
          </p>
        </div>
      )}
    </section>

...
export const AppShell: React.FC = () => {
  ...
  // Reset page canvas scroll position to the start whenever navigation occurs
  useEffect(() => {
    scrollToTop(false);
  ...
  return (
    <LiveStreamProvider>
      <div className="h-screen w-full bg-[#FAF8F3] dark:bg-[#0D1014] text-slate-900 dark:text-slate-100 flex flex-col font-sans antialiased overflow-hidden relative">
        {/* Continuous Ambient Radiance (Flowing seamlessly behind the top header and whole canvas) */}
        ...
        {/* Unified Institutional Top Navigation (Desktop, Tablet, Mobile) */}
        <TopNavigation />
        ...
        {/* Compact Mobile Navigation (Bottom Bar + More Sheet for Small Screens) */}
        <MobileNavigation />
      </div>
    </LiveStreamProvider>
  );
